...
import com.devops.agrishop.models.fcm.Data;
import com.devops.agrishop.models.fcm.FirebaseCloudMessage;
import com.devops.agrishop.utility.ChatMessageListAdapter;
import com.devops.agrishop.utility.FCM;
...
public class ChatroomActivity extends AppCompatActivity {
    ...
    private Set<String> mChatroomMembers;
...
//        getSupportActionBar().hide();
    ...
    private void init(){
        ...
        getServerKey();
        getChatroomMembers();
        //getUserTokens(mChatroomMembers);
    }
    ...
    private void getChatroomMembers(){
       // mUsersList = new ArrayList<>();
        mChatroomMembers = new HashSet<>();
        DatabaseReference dbref = FirebaseDatabase.getInstance().getReference();
        Query query = dbref.child("ENACTUS UOE").child(getString(R.string.dbnode_chatrooms))
                .child(mChatroom.getChatroom_id())
                .child("users");
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()){
                    for (DataSnapshot snapshot1: snapshot.getChildren()){
                        mChatroomMembers.add(snapshot1.getKey());
                        //mMessage.setText(mChatroomMembers.toString());
                        getUserTokens(mChatroomMembers);
                    }
                }
            }
    ...
    private void getUserTokens(Set<String> mChatroomMembers) {
        Log.d(TAG, "getUsersTokens: searching for tokens.");
        //mTokens.clear(); //clear current token list in case admin has change departments
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference();

        for(String userid: mChatroomMembers){
            Log.d(TAG, "getUserTokens: userid: " + userid);

            Query query = reference.child("ENACTUS UOE").child(getString(R.string.dbnode_users))
            ...
            query.addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(DataSnapshot dataSnapshot) {
                    for(DataSnapshot snapshot: dataSnapshot.getChildren()){
                        String token = snapshot.child("messaging_token").getValue().toString();
                        mTokens.add(token);
                        //mMessage.setText(mTokens.toString());
                    }
                }
    ...
    private void getChatroomMessages(){
        ...
        Query query = reference.child("ENACTUS UOE").child(getString(R.string.dbnode_chatrooms))
                .child(mChatroom.getChatroom_id())
                .child(getString(R.string.field_chatroom_messages));
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
    ...
    private void sendMessageToDepartment(String title, String message){
        ...
        for(String token :  mTokens){
            ...
            if (token != null){
                ...
                data.setChatroom_id(mChatroom.getChatroom_id());
                ...
                call.enqueue(new Callback<ResponseBody>() {
                    ...
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        ...
                        Toast.makeText(ChatroomActivity.this,"Success....msg send", Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onFailure(Call<ResponseBody> call, Throwable t) {
                        ...
                        Toast.makeText(ChatroomActivity.this, "Error, Try again", Toast.LENGTH_SHORT).show();
                    }
                });
            }else {
                Toast.makeText(ChatroomActivity.this, "Error, No token", Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void getServerKey(){
        ...
        Query query = reference.child("ENACTUS UOE").child(getString(R.string.dbnode_server))
                .orderByValue();
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
    ...
    private void initMessagesList(){
        mAdapter = new ChatMessageListAdapter(ChatroomActivity.this,
                R.layout.item_message_received, mMessagesList);
    ...
    */
    ...
    private void enableChatroomListener(){
         ...
         */
        mMessagesReference = FirebaseDatabase.getInstance().getReference().child("ENACTUS UOE").child(getString(R.string.dbnode_chatrooms))
                .child(mChatroom.getChatroom_id())
                .child(getString(R.string.field_chatroom_messages));
